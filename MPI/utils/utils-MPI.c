...
int readDenseCoordinateMatrixMPI(char *fileName,unsigned long **I,unsigned long **J, double **values,unsigned long *M,unsigned long *local_M,unsigned long *N, unsigned long long *nz, int myid, int numProcs) {
	...
	*values = (double *)  malloc(*nz * sizeof(double));
	...
	if(mm_read_mtx_crd_data_mpi(inputMatrix,*M,*N,*nz,*I,*J,*values,inputmatcode,startRow, endRow) != 0){
...
int mm_read_mtx_crd_data_mpi(FILE *f, unsigned long M, unsigned long N, unsigned long long nz, unsigned long I[], unsigned long J[],
        double val[], MM_typecode matcode, unsigned long startRow, unsigned long endRow, unsigned long long totalNz )
{
